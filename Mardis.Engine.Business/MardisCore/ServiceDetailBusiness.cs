...
namespace Mardis.Engine.Business.MardisCore
{
    ...
    public class ServiceDetailBusiness : ABusiness
    {
        ...
        public List<MyTaskServicesDetailViewModel> GetAnsweredSections(List<MyTaskServicesDetailViewModel> model, List<Answer> answers)
        {
            ...
            if (answers.Count() > 0) {
                answers = answers.OrderBy(x => x.sequenceSection).ToList();

            }
            foreach (var answer in answers)
            {
                if (answer.IdQuestion == Guid.Parse("2A0E1707-FD84-4F58-BC07-7A1F12071896"))
                {
                    var d = 0;
                }
                foreach (var answerDetail in answer.AnswerDetails)
                {
                    ...
                    foreach (var n in Section)
                    {
                        ...
                        //}
                            ...
                            pregunta = n.IsDynamic?  n?.QuestionCollection.Where(q => q.Id == answer.IdQuestion && q.sequence==answer.sequenceSection).FirstOrDefault(): n?.QuestionCollection.Where(q => q.Id == answer.IdQuestion).FirstOrDefault();
                    ...
                    for (i = 0; i < questionList.Count(); i++)
                    {
                        if (questionList[i] != null)
                        {
                            if (questionList.Count > 1)
                                answer.Question.sequence = answer.sequenceSection;
                            else
                                    ...
                                    switch (answer.Question.TypePoll.Code)
                                    {
                                        ...
                                        case CTypePoll.Many:
                                            ...
                                            if (question.QuestionDetailCollection.Where(x => x.Id == answerDetail.IdQuestionDetail).Count() > 0)
                                            {
                                                ...
                                                if (question.HasPhoto == "D") {
                                                   var _model= _answerDetailDao.GetAnswerDetailSecond(answerDetail.Id);

                                                    question.QuestionDetailCollection.Where(x => x.Id == answerDetail.IdQuestionDetail).First().AnwerDetailSecondModel =  _model;
                                                    question.QuestionDetailCollection.Where(x => x.Id == answerDetail.IdQuestionDetail).First().AnwerDetailSecondModel.First().QuestionComplete=_model.First().QuestionComplete;
                                                }
